import type { Connect, IndexHtmlTransform, PluginOption, ResolvedConfig } from 'vite'
...
export function createPlugin(userOptions: UserOptions = {}): PluginOption {
  ...
  const transformIndexHtmlHandler: IndexHtmlTransform = async (html, ctx) => {
    ...
    const htmlName = path.relative(cwd, excludeBaseUrl)
  ...
  return {
    ...
    transformIndexHtml:
      getViteMajorVersion() >= 5
        ? {
            // @ts-expect-error old version vite types
            order: 'pre',
...
export function getPageConfig(htmlName: string, pages: Pages, defaultPage: string): PageOption {
  ...
  const page = pages.filter((page) => {
    // console.log(path.resolve('/' + page.template), path.resolve('/' + htmlName))
...
function createRewire(reg: string, page: PageOption, baseUrl: string, proxyUrlKeys: string[]) {
  ...
  return {
    ...
    to({ parsedUrl }: { parsedUrl: { path: string } }) {
