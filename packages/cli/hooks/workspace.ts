...
function isCurrentDirectoryAvailable(root: string, packages: RawPackage[]) {
  ...
  if (!isRoot && !isPackage)
    throw new Error(logger.NOT_AVAILABLE_PATH)
...
async function packagesFilter(root: string, names: string[] | undefined, packages: BasePackage[], all: boolean = false, diff: boolean = false, prompt: boolean = false): Promise<BasePackage[]> {
  const isRoot = !packages.some(pkg => pathOperator.pathsEqual(pkg.path, dict.cwd))
  // Only when not in the root directory, and in the package directory, will it return only the packages in the current directory.
  if (!isRoot) {
    const currentPackage = packages.filter(pkg => pathOperator.pathsEqual(pkg.path, dict.cwd))
    if (!currentPackage.length)
      throw new Error(logger.PACKAGE_MISSING)
  ...
  const filteredPackages = packages.filter(pkg =>
    names.some(name =>
      name.includes('@') ? name === pkg.name : name === basename(pkg.path),
    ),
  )
...
function mappedPackages(packages: RawPackage[], root: string): BasePackage[] {
  return packages.map(pkg => ({
    ...
    relativePath:
      relative(dict.cwd, pkg.path) === '' ? './' : relative(dict.cwd, pkg.path),
  }))
}
...
export default async (
  ...
  if (!npmClient)
    throw new Error(logger.PACKAGE_MANAGER_NOT_FOUND)

  const pkgs = mappedPackages(packages, workspace.path)

  const filtedPackages = await packagesFilter(
    workspace.path,
    names,
    pkgs,
    all,
